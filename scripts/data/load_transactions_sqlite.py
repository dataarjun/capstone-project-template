"""
Load Transactions Data into SQLite Database

This script loads the transactions.csv file into SQLite and provides functions
to retrieve data for the AML agents.
"""

import pandas as pd
import sqlite3
from datetime import datetime
import os
from pathlib import Path
import sys

# Add project root to path
project_root = Path(__file__).parent
sys.path.append(str(project_root))

class SQLiteTransactionLoader:
    """Load and manage transaction data in SQLite"""
    
    def __init__(self, db_path: str = "data/transactions.db"):
        self.db_path = db_path
        self.connection = None
    
    def connect(self):
        """Connect to SQLite database"""
        try:
            # Ensure directory exists
            os.makedirs(os.path.dirname(self.db_path), exist_ok=True)
            
            self.connection = sqlite3.connect(self.db_path)
            self.connection.row_factory = sqlite3.Row  # Enable column access by name
            print(f"✅ Connected to SQLite database: {self.db_path}")
        except Exception as e:
            print(f"❌ Failed to connect to SQLite: {e}")
            raise
    
    def disconnect(self):
        """Disconnect from SQLite database"""
        if self.connection:
            self.connection.close()
            print("✅ Disconnected from SQLite database")
    
    def create_transactions_table(self):
        """Create transactions table if it doesn't exist"""
        statements = [
            """CREATE TABLE IF NOT EXISTS transactions (
                transaction_id INTEGER PRIMARY KEY,
                sender_account_id INTEGER NOT NULL,
                receiver_account_id INTEGER NOT NULL,
                tx_type TEXT NOT NULL,
                amount REAL NOT NULL,
                timestamp INTEGER NOT NULL,
                is_fraud INTEGER NOT NULL DEFAULT 0,
                alert_id INTEGER DEFAULT -1,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )""",
            "CREATE INDEX IF NOT EXISTS idx_transactions_sender ON transactions(sender_account_id)",
            "CREATE INDEX IF NOT EXISTS idx_transactions_receiver ON transactions(receiver_account_id)",
            "CREATE INDEX IF NOT EXISTS idx_transactions_amount ON transactions(amount)",
            "CREATE INDEX IF NOT EXISTS idx_transactions_timestamp ON transactions(timestamp)",
            "CREATE INDEX IF NOT EXISTS idx_transactions_fraud ON transactions(is_fraud)"
        ]
        
        try:
            for statement in statements:
                self.connection.execute(statement)
            self.connection.commit()
            print("✅ Transactions table created/verified")
        except Exception as e:
            print(f"❌ Failed to create transactions table: {e}")
            raise
    
    def load_transactions_from_csv(self, csv_path: str, batch_size: int = 10000):
        """Load transactions from CSV file into SQLite"""
        print(f"📥 Loading transactions from {csv_path}")
        
        # Check if file exists
        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"CSV file not found: {csv_path}")
        
        # Get file size for progress tracking
        file_size = os.path.getsize(csv_path)
        print(f"📊 File size: {file_size / (1024*1024):.1f} MB")
        
        # Read CSV in chunks
        total_rows = 0
        chunk_count = 0
        
        try:
            for chunk in pd.read_csv(csv_path, chunksize=batch_size):
                chunk_count += 1
                
                # Prepare data for insertion
                records = []
                for _, row in chunk.iterrows():
                    records.append((
                        int(row['transaction_id']),
                        int(row['SENDER_ACCOUNT_ID']),
                        int(row['RECEIVER_ACCOUNT_ID']),
                        str(row['TX_TYPE']),
                        float(row['amount']),
                        int(row['TIMESTAMP']),
                        1 if bool(row['IS_FRAUD']) else 0,  # Convert boolean to int for SQLite
                        int(row['ALERT_ID'])
                    ))
                
                # Insert batch into database
                insert_sql = """
                INSERT OR REPLACE INTO transactions 
                (transaction_id, sender_account_id, receiver_account_id, tx_type, amount, timestamp, is_fraud, alert_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """
                
                self.connection.executemany(insert_sql, records)
                self.connection.commit()
                total_rows += len(records)
                
                if chunk_count % 50 == 0:  # Progress update every 50 chunks
                    print(f"📈 Processed {chunk_count} chunks, {total_rows:,} transactions loaded")
            
            print(f"✅ Successfully loaded {total_rows:,} transactions")
            return total_rows
            
        except Exception as e:
            print(f"❌ Error loading transactions: {e}")
            raise
    
    def get_transaction_stats(self):
        """Get statistics about loaded transactions"""
        try:
            cursor = self.connection.execute("""
                SELECT 
                    COUNT(*) as total_transactions,
                    COUNT(DISTINCT sender_account_id) as unique_senders,
                    COUNT(DISTINCT receiver_account_id) as unique_receivers,
                    MIN(amount) as min_amount,
                    MAX(amount) as max_amount,
                    AVG(amount) as avg_amount,
                    COUNT(CASE WHEN is_fraud = 1 THEN 1 END) as fraud_count,
                    MIN(created_at) as earliest_transaction,
                    MAX(created_at) as latest_transaction
                FROM transactions
            """)
            
            stats = cursor.fetchone()
            
            print("📊 Transaction Statistics:")
            print(f"   Total Transactions: {stats['total_transactions']:,}")
            print(f"   Unique Senders: {stats['unique_senders']:,}")
            print(f"   Unique Receivers: {stats['unique_receivers']:,}")
            print(f"   Amount Range: ${stats['min_amount']:.2f} - ${stats['max_amount']:.2f}")
            print(f"   Average Amount: ${stats['avg_amount']:.2f}")
            print(f"   Fraud Transactions: {stats['fraud_count']:,}")
            print(f"   Date Range: {stats['earliest_transaction']} to {stats['latest_transaction']}")
            
            return dict(stats)
            
        except Exception as e:
            print(f"❌ Error getting transaction stats: {e}")
            return None

class SQLiteTransactionRetriever:
    """Retrieve transaction data for AML agents from SQLite"""
    
    def __init__(self, db_path: str = "data/transactions.db"):
        self.db_path = db_path
        self.connection = None
    
    def connect(self):
        """Connect to SQLite database"""
        try:
            self.connection = sqlite3.connect(self.db_path)
            self.connection.row_factory = sqlite3.Row
            print(f"✅ Connected to SQLite database for retrieval: {self.db_path}")
        except Exception as e:
            print(f"❌ Failed to connect to SQLite: {e}")
            raise
    
    def disconnect(self):
        """Disconnect from SQLite database"""
        if self.connection:
            self.connection.close()
    
    def get_transactions_for_agents(self, limit: int = 100, offset: int = 0, 
                                  min_amount: float = None, max_amount: float = None,
                                  fraud_only: bool = False):
        """
        Retrieve transactions formatted for AML agents
        
        Args:
            limit: Maximum number of transactions to retrieve
            offset: Number of transactions to skip
            min_amount: Minimum transaction amount filter
            max_amount: Maximum transaction amount filter
            fraud_only: If True, only return fraud transactions
        
        Returns:
            List of transaction dictionaries formatted for agents
        """
        try:
            # Build query
            where_conditions = []
            params = []
            
            if min_amount is not None:
                where_conditions.append("amount >= ?")
                params.append(min_amount)
            
            if max_amount is not None:
                where_conditions.append("amount <= ?")
                params.append(max_amount)
            
            if fraud_only:
                where_conditions.append("is_fraud = 1")
            
            where_clause = "WHERE " + " AND ".join(where_conditions) if where_conditions else ""
            
            params.extend([limit, offset])
            
            query = f"""
                SELECT 
                    transaction_id,
                    sender_account_id,
                    receiver_account_id,
                    tx_type,
                    amount,
                    timestamp,
                    is_fraud,
                    alert_id,
                    created_at
                FROM transactions 
                {where_clause}
                ORDER BY transaction_id
                LIMIT ? OFFSET ?
            """
            
            cursor = self.connection.execute(query, params)
            rows = cursor.fetchall()
            
            # Convert to agent-friendly format
            transactions = []
            for row in rows:
                transaction = {
                    "transaction_id": str(row['transaction_id']),  # Convert to string for agents
                    "sender_account_id": str(row['sender_account_id']),
                    "receiver_account_id": str(row['receiver_account_id']),
                    "tx_type": str(row['tx_type']),
                    "amount": float(row['amount']),
                    "timestamp": int(row['timestamp']),
                    "is_fraud": bool(row['is_fraud']),
                    "alert_id": int(row['alert_id']),
                    "created_at": row['created_at']
                }
                transactions.append(transaction)
            
            print(f"📥 Retrieved {len(transactions)} transactions for agents")
            return transactions
            
        except Exception as e:
            print(f"❌ Error retrieving transactions: {e}")
            return []
    
    def get_transaction_by_id(self, transaction_id: str):
        """Get a specific transaction by ID"""
        try:
            cursor = self.connection.execute("""
                SELECT 
                    transaction_id,
                    sender_account_id,
                    receiver_account_id,
                    tx_type,
                    amount,
                    timestamp,
                    is_fraud,
                    alert_id,
                    created_at
                FROM transactions 
                WHERE transaction_id = ?
            """, (int(transaction_id),))
            
            row = cursor.fetchone()
            
            if row:
                return {
                    "transaction_id": str(row['transaction_id']),
                    "sender_account_id": str(row['sender_account_id']),
                    "receiver_account_id": str(row['receiver_account_id']),
                    "tx_type": str(row['tx_type']),
                    "amount": float(row['amount']),
                    "timestamp": int(row['timestamp']),
                    "is_fraud": bool(row['is_fraud']),
                    "alert_id": int(row['alert_id']),
                    "created_at": row['created_at']
                }
            return None
            
        except Exception as e:
            print(f"❌ Error retrieving transaction {transaction_id}: {e}")
            return None

def main():
    """Main function to load transactions and test retrieval"""
    print("🚀 Loading Transactions into SQLite Database")
    print("=" * 60)
    
    # Initialize loader
    loader = SQLiteTransactionLoader()
    
    try:
        # Connect to database
        loader.connect()
        
        # Create table
        loader.create_transactions_table()
        
        # Load transactions from CSV
        csv_path = "/Users/indrajitsingh/Course_Materials/AgenticAI/Capstone2025/data/transactions.csv"
        total_loaded = loader.load_transactions_from_csv(csv_path, batch_size=10000)
        
        # Get statistics
        loader.get_transaction_stats()
        
        # Test retrieval
        print("\n🧪 Testing Transaction Retrieval")
        print("=" * 40)
        
        retriever = SQLiteTransactionRetriever()
        retriever.connect()
        
        # Test getting sample transactions
        sample_transactions = retriever.get_transactions_for_agents(limit=10)
        print(f"📥 Retrieved {len(sample_transactions)} sample transactions")
        
        if sample_transactions:
            print("📋 Sample transaction:")
            sample = sample_transactions[0]
            for key, value in sample.items():
                print(f"   {key}: {value} (type: {type(value).__name__})")
        
        # Test getting fraud transactions
        fraud_transactions = retriever.get_transactions_for_agents(limit=5, fraud_only=True)
        print(f"🚨 Retrieved {len(fraud_transactions)} fraud transactions")
        
        # Test getting high-value transactions
        high_value_transactions = retriever.get_transactions_for_agents(
            limit=5, min_amount=10000.0
        )
        print(f"💰 Retrieved {len(high_value_transactions)} high-value transactions")
        
        retriever.disconnect()
        
        print("\n✅ Transaction loading and retrieval completed successfully!")
        
    except Exception as e:
        print(f"❌ Error in main process: {e}")
        import traceback
        print(traceback.format_exc())
    
    finally:
        loader.disconnect()

if __name__ == "__main__":
    main()
